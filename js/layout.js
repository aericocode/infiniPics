/**
 * InfiniPics - Layout Management
 * Grid layout calculation and image resizing
 */

function recalculateLayout() {
    const container = document.getElementById('imageContainer');
    const count = State.imageSlots.length;
    if (count === 0) return;
...
function updateScalingMode() {
    ...
    State.imageSlots.forEach((slot, i) => {
        document.getElementById(`content-${i}`).className = `image-content ${State.scalingMode}`;
    });
}
...
function doResize(event) {
    ...
    State.imageSlots[State.resizing.index].gridSpan = { col: colSpan, row: rowSpan };
    
    const wrapper = document.getElementById(`imageWrapper-${State.resizing.index}`);
...
function stopResize() {
    if (State.resizing) {
        document.getElementById(`imageWrapper-${State.resizing.index}`).classList.remove('resizing');
    }
    State.resizing = null;
    ...
    document.addEventListener('dblclick', (e) => {
        if (e.target.classList.contains('resize-handle')) {
            ...
            if (wrapper) {
                ...
                State.imageSlots[index].gridSpan = { col: 1, row: 1 };
